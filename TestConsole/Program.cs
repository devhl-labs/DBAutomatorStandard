...
#nullable enable
        ...
        {
            SqlWriter = new SqlWriter(new SqlWriterConfiguration());

            SqlWriter.AddPropertyMap(new PropertyMap(typeof(ulong), ObjectToString));  // these depend on what column types and RDMS you use

            SqlWriter.AddPropertyMap(new PropertyMap(typeof(ulong?), ObjectToString));

            SqlWriter.AddPropertyMap(new PropertyMap(typeof(long), ObjectToString));
        ...
        private static object? ObjectToString(object? arg) => arg?.ToString() ?? null;       

        public static void RegisterClasses()
        {
            SqlWriter.Register<UserModel>()
                     .Key(u => u.UserID)                                  // primary key
                     .NotMapped(u => u.NotMappedProperty)                 // this property is not in the database
                     .ColumnName(u => u.SomeLongNumber!, "SomeBigInt");   // this property name does not match the column name

            SqlWriter.Register<AddressModel>();

            var userAddressesJoin = SqlWriter.Register<User_WithAddresses>()
                                             .Key(u => u.UserID)
                                             .NotMapped(u => u.NotMappedProperty)
                                             .ColumnName(u => u.SomeLongNumber!, "SomeBigInt");

            // this instructs on how to break the joined row into nested objects
            userAddressesJoin.QueryAsync = User_WithAddresses.AddAddressesToList;
        }   
        ...
        private static async Task TestUsingConstants()
        {
            ...
            var c = await SqlWriter.Select<UserModel>().Limit(2).Where<UserModel>(u => u.UserType == UserType.User).QueryAsync();

            var d = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.FirstName == "Adam").QueryFirstAsync();

            var e = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.UserType == UserType.Admin && u.SomeLongNumber == 1000 && (u.FirstName == "Debra" || u.FirstName == "Dexter")).QueryAsync();

            var f = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.SomeLongNumber > 0).QueryAsync();

            var g = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.SomeLongNumber > 0 || (u.FirstName == "Adam" && u.LastName == "Warlock") || u.FirstName == "Fool").QueryAsync();
        ...
        private static async Task TestUsingVariableClosure()
        {
            ...
            var c = await SqlWriter.Select<UserModel>().Limit(two).Where<UserModel>(u => u.UserType == userTypeZero).QueryAsync(); //casting in the Where clause is not supported by the Mia Plaza library

            var d = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.FirstName == adam).QueryFirstAsync();

            var e = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.UserType == userTypeOne && u.SomeLongNumber == onethousand && (u.FirstName == debra || u.FirstName == dexter)).QueryAsync();

            var f = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.SomeLongNumber > zero).QueryAsync();

            var g = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.SomeLongNumber > zero || (u.FirstName == adam && u.LastName == warlock) || u.FirstName == fool).QueryAsync();
        ...
        private static async Task TestUsingComplexClosure()
        {
            ...
            var c = await SqlWriter.Select<UserModel>().Limit(2).Where<UserModel>(u => u.UserType == adam.UserType).QueryAsync();

            var d = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.FirstName == adam.FirstName).QueryFirstAsync();

            var e = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.UserType == dexter.UserType && u.SomeLongNumber == dexter.SomeLongNumber && (u.FirstName == debra.FirstName || u.FirstName == dexter.FirstName)).QueryAsync();

            var f = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.SomeLongNumber > adam.SomeLongNumber.Value).QueryAsync();            

            var g = await SqlWriter.Select<UserModel>().Where<UserModel>(u => u.SomeLongNumber > adam.SomeLongNumber.Value || (u.FirstName == adam.FirstName && u.LastName == adam.LastName) || u.FirstName == fool.FirstName).QueryAsync();
        ...
        private static async Task TestWithJoins()
        {
            var a = await SqlWriter.Delete<UserModel>().QueryAsync();

            await InsertNewRows();

            var b = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .Join<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .QueryAsync();

            var c = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .LeftJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .QueryAsync();

            var d = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .RightJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .QueryAsync();

            var e = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .Join<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<User_WithAddresses>(u => u.FirstName == "Debra")
                                   .QueryAsync();

            var f = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .LeftJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<User_WithAddresses>(u => u.FirstName == "Debra")
                                   .QueryAsync();

            var g = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .RightJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<User_WithAddresses>(u => u.FirstName == "Debra")
                                   .QueryAsync();

            var h = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .Join<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<AddressModel>(a => a.UserAddress == "another new world")
                                   .QueryAsync();

            var i = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .LeftJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<AddressModel>(a => a.UserAddress == "another new world")
                                   .QueryAsync();

            var j = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .RightJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<AddressModel>(a => a.UserAddress == "another new world")
                                   .QueryAsync();

            var k = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .Join<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<AddressModel>(a => a.UserAddress == "another new world")
                                   .Where<User_WithAddresses>(u => u.FirstName == "Adam")
                                   .QueryAsync();

            var l = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .LeftJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<AddressModel>(a => a.UserAddress == "another new world")
                                   .Where<User_WithAddresses>(u => u.FirstName == "Adam")
            ...
            var m = await SqlWriter.Select<User_WithAddresses>()
                                   .Select<AddressModel>()
                                   .RightJoin<User_WithAddresses, AddressModel>((u, a) => u.UserID == a.UserID)
                                   .Where<AddressModel>(a => a.UserAddress == "another new world")
                                   .Where<User_WithAddresses>(u => u.FirstName == "Adam")
    ...
    public class SqlWriterConfiguration : ISqlWriterConfiguration
    {
        ...
        public IDbConnection CreateDbConnection()
        {
            ...
            return new NpgsqlConnection($"Server=127.0.0.1;Port=5432;Database=AutomatorTest;User ID=postgres;Password={password};");
        }
    }
}
